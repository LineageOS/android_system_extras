...
class SparseWriter final {
  public:
    SparseWriter(int output_fd, uint32_t block_size);

    bool WriteExtent(int image_fd, const LpMetadataExtent& extent);
    bool Finish();
...
static int usage(int /* argc */, char* argv[]) {
    fprintf(stderr,
            "%s - command-line tool for extracting partition images from split super\n"
            "\n"
            "Usage:\n"
...
int main(int argc, char* argv[]) {
    ...
    std::vector<std::string> image_files(1);
...
SparseWriter::SparseWriter(int output_fd, uint32_t block_size)
    : output_fd_(output_fd), block_size_(block_size) {}

bool SparseWriter::WriteExtent(int image_fd, const LpMetadataExtent& extent) {
    auto buffer = std::make_unique<uint8_t[]>(block_size_);

    off_t super_offset = extent.target_data * LP_SECTOR_SIZE;
    if (lseek(image_fd, super_offset, SEEK_SET) < 0) {
